...
function submitQuote(event) {
    ...
    const formType = selectedQuoteType;
    const formConfig = quoteForms[formType];
    
    // Create email content
    let emailContent = `Type de demande: ${formConfig.title}\n\n`;
    
    // Add form fields to email content
    formConfig.fields.forEach(field => {
        if (field.type === 'section') {
            emailContent += `${field.label}:\n`;
            field.fields.forEach(subField => {
                const value = formData.get(subField.name);
                if (value) emailContent += `- ${subField.label}: ${value}\n`;
            });
            emailContent += '\n';
        } else if (field.type === 'radio' || field.type === 'select') {
            const value = formData.get(field.name);
            const option = field.options?.find(opt => opt.value === value);
            emailContent += `${field.label}: ${option ? option.label : value}\n\n`;
        } else if (field.type === 'checkbox') {
            const values = formData.getAll(field.name + '[]');
            const selectedOptions = field.options?.filter(opt => values.includes(opt.value));
            emailContent += `${field.label}: ${selectedOptions?.map(opt => opt.label).join(', ') || 'Aucune'}\n\n`;
        } else {
            const value = formData.get(field.name);
            if (value) emailContent += `${field.label}: ${value}\n\n`;
        }
    });
    
    // Send email using FormSubmit or similar service
    const emailData = {
        to: 'contact@partenaireassurances.com',
        subject: `Nouvelle demande de devis - ${formConfig.title}`,
        body: emailContent,
        from: formData.get('email') || 'noreply@gpa-assurances.fr'
    };
...
function submitContact(event) {
    ...
    // Collect form data
    const formData = new FormData(event.target);
    const name = formData.get('name') || 'Non renseigné';
    const email = formData.get('email') || 'Non renseigné';
    const phone = formData.get('phone') || 'Non renseigné';
    const message = formData.get('message') || 'Non renseigné';
    
    // Create email content
    const emailContent = `Nouveau message de contact\n\n` +
        `Nom: ${name}\n` +
        `Email: ${email}\n` +
        `Téléphone: ${phone}\n` +
        `Message: ${message}\n\n` +
        `Envoyé depuis: GPA Assurances Website`;
    ...
    const emailData = {
        ...
        body: emailContent,
        from: email || 'noreply@gpa-assurances.fr'
    };
